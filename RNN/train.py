...
from modelfile import AttnDecoderRNN, EncoderRNN
...
def train(encoder, decoder, n_iters, pairs, input_lang, output_lang, print_every=1000, plot_every=1000, learning_rate=0.01):
    ...
    for iter_ in range(1, n_iters + 1):
        ...
        loss = train_iteration(input_tensor.to(device), target_tensor.to(device), encoder,
...
def main():
    ...
    for i in range(epoch):
        attn_decoder.train()
        losses = train(encoder, attn_decoder, len(pairs_train), pairs=pairs_train, input_lang=input_lang, output_lang=output_lang, print_every=10)
        loss_history.extend(losses)
        attn_decoder.eval()
        evaluate_all(encoder, attn_decoder, pairs_test, max_length=1000, input_lang=input_lang, output_lang=output_lang, n = len(pairs_test))
